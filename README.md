...
3. **Admin Exclusivity Override:** I assumed that AI isn't infallible. Thus, I unlocked the Django `read_only_fields` for `category` and `priority` *only* for PATCH requests. I then built inline dropdown overrides in the UI specifically visible to `isAdmin`, assuming BookLeaf operators must maintain absolute final authority over API-driven routing decisions.
